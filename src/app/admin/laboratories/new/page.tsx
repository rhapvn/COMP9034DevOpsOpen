...
import { addLab } from "@/db/apiRoutes";
...
const modalInitialProperty = {
  ...
  description: "Your laboratory details are successfully saved.",
  btnCancel: true,
  btnCancelCol: "button",
...
export default function Page() {
  ...
  const handlePageCancel = () => {
    dispatch({ type: "reset" });
    router.push("/admin/laboratories"); // should be redirect back to the main page
  };
  const handlePageSubmit = async () => {
    if (!name || !address || !centreId) {
      alert("Name, Address and Centre cannot be empty!");
      return;
    }
    const newLab: Partial<Laboratory> = {
      centreId,
      name,
      address,
    };
  ...
  return (
    <div className="mx-4">
      <TitleLine name="Add New Laboratory" />
      ...
      {isLoading ? (
        ...
        <div className="mt-[60px] flex w-full flex-col items-center justify-center gap-y-4">
          <InputField
            label="Place Name"
            placeholder="Place enter laboratory name"
            value={name}
          ...
          <InputField
            ...
            className="w-[500px]"
          />
          <CustomDropDown type="Research" setOrganization={setCentreId} />
        </div>
      )}
